...
int main(int argc, const char* argv[]) {
    ...
    options_description assembly("Assembly options: genetic code must be specified");
    assembly.add_options()
        ...
        ("word", value<int>()->default_value(12), "Word size for seeds [integer <= 16]")
        ...
        ("gap_open", value<int>()->default_value(11), "Penalty for gap opening [integer]")
        ("gap_extend", value<int>()->default_value(1), "Penalty for gap extension [integer]")
        ("allow_frameshifts", "Allow frameshifts [flag]")
        ("fs_open", value<int>()->default_value(25), "Penalty for opening a gap that results in a frameshift [integer]")
        ...
        ("genetic_code", value<int>(), "Genetic code [integer]");
    ...
    try {
...
#endif
            cout << all << "\n";
            cout << "Accepted genetic codes:" << endl;
            GeneticCode gcode;
            for(auto& info : gcode.AllInfo())
                cout << "      " << get<2>(info) << " " << get<3>(info) << endl;
            
            return 0;
        }

        if(argm.count("version")) {
            cout << "saute_prot 1.3.0" << endl;
#ifdef SVN_REV
        ...
        if(!argm.count("genetic_code")) {
            cerr << "Provide genetic code" << endl;
            cerr << all << "\n";
            cerr << "Accepted genetic codes:" << endl;
            GeneticCode gcode;
            for(auto& info : gcode.AllInfo())
                cerr << "      " << get<2>(info) << " " << get<3>(info) << endl;
            return 1;
        }
        int genetic_code =  argm["genetic_code"].as<int>();
        try {
            GeneticCode gcode(genetic_code);
        } catch (exception &e) {
        ...
        int fs_open = argm["fs_open"].as<int>();
        int word_size = min(15, argm["word"].as<int>());
        ...
        if(argm.count("kmer")) {
            ...
            if(kmer%2 ==0 || kmer%3 != 0) {
                cerr << "Kmer must be an odd number divisible by 3" << endl;
                return 1;
            }
            secondary_kmer = argm["secondary_kmer"].as<int>();
            if(secondary_kmer > kmer || secondary_kmer%2 == 0 || secondary_kmer%3 != 0) {
                cerr << "Secondary kmer must be an odd number divisible by 3 smaller or equal kmer" << endl;
                return 1;
            }
        }
        ...
        CReadsGetter readsgetter(sra_list, file_list, ncores, usepairedends);
        {
            ...
            if(kmer == 0) {
                ...
                if(kmer%2 == 0)
                    --kmer;
                while(kmer%3 != 0 && kmer > 3)
                    kmer -= 2;
                secondary_kmer = read_len/5;
                if(secondary_kmer%2 == 0)
                    --secondary_kmer;
                while(secondary_kmer%3 != 0 && secondary_kmer > 3)
                    secondary_kmer -= 2;
            ...
            cerr << "Read length: " << read_len << " Kmer: " << kmer << " Secondary kmer: " << secondary_kmer << endl;
        }
        ...
        bool translate_na = argm.count("na_targets");
        bool allow_frameshifts = argm.count("allow_frameshifts");
        ...
        CGuidedAssemblerAA gassembler(kmer, secondary_kmer, extend_ends, protect_reference_ends, keep_subgraphs, min_count, fraction, word_size, gap_open, gap_extend, drop_off, 
                                      kmer_complexity, max_fork_density, buf_length, ncores, reads, targets_in, estimated_kmer_num, skip_bloom_filter, not_aligned_len, 
                                      not_aligned_count, aligned_count, maxp, target_coverage, min_hit_len, no_reads, no_pairs, 
                                      genetic_code, translate_na, fs_open, allow_frameshifts, secondary_kmer_threshold, remove_homopolymer_indels, homopolymer_len, homopolymer_ratio);
